...
CodeMirror.defineMode('markdown', (cmCfg, modeCfg) => {
  ...
  for (const tokenType in tokenTypes) {
    if (
      Object.prototype.hasOwnProperty.call(tokenTypes, tokenType) &&
  ...
  function switchInline(stream: CodeMirror.StringStream, state: any, f: any) {
    state.f = state.inline = f
    return f(stream, state)
  }

  function switchBlock(stream: CodeMirror.StringStream, state: any, f: any) {
    state.f = state.block = f
    return f(stream, state)
  }

  function lineIsEmpty(line: CodeMirror.StringStream) {
    return !line || !/\S/.test(line.string)
  }

  // Blocks

  function blankLine(state: any) {
  ...
  function blockNormal(stream: CodeMirror.StringStream, state: any) {
  ...
  function htmlBlock(stream: CodeMirror.StringStream, state: any) {
    ...
    if (!htmlModeMissing) {
      ...
      if (
        (inner.mode.name === 'xml' &&
          ...
          !inner.state.context &&
          inner.state.tokenize.isInText) ||
        (state.md_inside && stream.current().includes('>'))
      ) {
        state.f = inlineNormal
        state.block = blockNormal
        state.htmlState = null
  ...
  function local(stream: CodeMirror.StringStream, state: any) {
      ...
      for (let i = 0; i < state.formatting.length; i++) {
        ...
        if (state.formatting[i] === 'quote') {
          if (
            ...
            styles.push(
              `${tokenTypes.formatting}-${state.formatting[i]}-${state.quote}`,
            )
          } else {
            styles.push('error')
          }
  ...
  function handleText(stream: CodeMirror.StringStream, state: any) {
  ...
  function inlineNormal(stream: CodeMirror.StringStream, state: any) {
    ...
    if (
      ...
      if (end !== -1) {
        const atts = stream.string.slice(stream.start, end)
    ...
    if (
      modeCfg.emoji &&
      ch === ':' &&
      stream.match(/^(?:[\d+_a-z][\d+_a-z-]*|-[\d+_a-z][\d+_a-z-]*):/)
    ) {
      state.emoji = true
  ...
  function linkInline(stream: CodeMirror.StringStream, state: any) {
    const ch = stream.next()
  ...
  function linkHref(stream: CodeMirror.StringStream, state: any) {
  ...
  function getLinkHrefInside(endChar: ')' | ']') {
    return function (stream: CodeMirror.StringStream, state: any) {
      const ch = stream.next()
  ...
  function footnoteLink(stream: CodeMirror.StringStream, state: any) {
    if (stream.match(/^([^\\\]]|\\.)*]:/, false)) {
  ...
  function footnoteLinkInside(stream: CodeMirror.StringStream, state: any) {
  ...
  function footnoteUrl(stream: CodeMirror.StringStream, state: any) {
    // Check if space, and return NULL if so (to avoid marking the space)
    if (stream.eatSpace()) {
      return null
    }
  ...
  const mode = {
    ...
    indent(state: any, textAfter: string, line: string) {
